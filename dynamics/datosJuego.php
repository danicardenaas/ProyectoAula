<?php
    ...
    $juego = (isset($_POST['nombreJuego']) && $_POST["nombreJuego"] != "")? $_POST['nombreJuego'] : false;
    $fechaLimite = (isset($_POST['fecha']) && $_POST["fecha"] != "")? $_POST['fecha'] : false;
    $ID_materia = (isset($_POST['ID_materia']) && $_POST["ID_materia"] != "")? $_POST['ID_materia'] : false;
    // $peticion = "INSERT INTO juego (ID_juego) VALUES ('$pregunta[$i]', '$ID_juego')";
    // $pk_pregunta=  mysqli_insert_id($conexion);
    ...
    if($juego && $fechaLimite)
    {
        ...
        for($i=0; $i<10; $i++)
        {
            ...
            else
            {
                ...
                else 
                {
                    ...
                    $ruta="../Imgs/juegos/predeterminada.png"; //predeterminada
                }
        ...
        $peticion = "INSERT INTO actividad (fecha_pub, ID_materia, ID_juego, ID_entrega, nombre, fecha_limite) VALUES ('$fecha', $ID_materia, $ID_juego, '2', '$juego', '$fechaLimite')"; //verificador 0
        $query = mysqli_query($conexion, $peticion); 
        $resultados= array ("ok"=>true, "texto"=> "Registrado"); 
    }
    else 
    {
        $query = "Llena todos los campos"; 
        $resultados= array ("ok"=>false, "texto"=>"Llena todos los campos"); 
    }
